...
from util import Configuration, Connect
...
class RecordToDatabase(object):
    def __init__(self):
        ...
        self.ip = Configuration.configs["LocalIP"]
        ...
        for line in dataFile:
            ...
            if sqlDict['rtype'] == '24':
                sql = "insert into trace_info(on_ip,on_port,in_ip,in_port,data_id,data_unit_id,pid,ktid,tid,ftype,ltime,length,supposed_length,rlength,connect_id,dtype,message,message2,source_ip)" \
                      ...
                      (sqlDict['on_ip'], sqlDict['on_port'], sqlDict['in_ip'], sqlDict['in_port'], sqlDict['uuid'],
                       sqlDict['unit_uuid'],
                       sqlDict['pid'], sqlDict['ktid'], sqlDict['tid'], sqlDict['ftype'], sqlDict['ttime'],
                       sqlDict['length'], sqlDict['supposed_length'],
                ...
                sql = "insert into threads(process_id,k_thread_id,thread_id,p_process_id,pk_thread_id,p_thread_id,ltime,data_unit_id,source_ip) values " \
                      "(%s,%s,%s,%s,%s,%s,%s,'%s','%s');" % (
                      sqlDict['pid'], sqlDict['ktid'], sqlDict['tid'], sqlDict['ppid'], sqlDict['pktid'],
                      sqlDict['ptid'], sqlDict['ttime'], sqlDict['unit_uuid'], self.ip)
                ...
                sql = "insert into thread_dep(process_id,k_thread_id,d_thread_id,j_thread_id,ltime,source_ip,data_unit_id) values" \
                      "(%s,%s,%s,%s,%s,'%s','%s');" % (
                      sqlDict['pid'], sqlDict['ktid'], sqlDict['dtid'], sqlDict['jtid'], sqlDict['ttime'], self.ip,
                      sqlDict['unit_uuid'])
            elif sqlDict['rtype'] == '29':
                sql = "insert into other_event(ftype,result,ltime,pid,ktid,tid,data_unit_id,source_ip) values (%s,%s,%s,%s,%s,%s,'%s','%s')" % (
                sqlDict['ftype'], sqlDict['result'], sqlDict['ltime'],
...
if __name__ == '__main__':
    r = RecordToDatabase()
    r.recordToDatabase(Configuration.configs['TraceDataPath'])
