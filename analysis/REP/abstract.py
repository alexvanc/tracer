...
class Abstract():
    def __init__(self):
        ...
        self.source = "combination1"
        self.target = "abstract1"
    ...
    def import_all(self):
        ...
        for result in results:
            sql2 = "select id,source_type,ftype from %s where data_unit_id='%s' and ktid=%s and source_ip='%s'" % (
            self.source, result[0], result[1], result[2])
            ...
            sql4 = "insert into %s(data_unit_id,ktid,source_ip,internal_length,child_number,f_sequence,source_sequence) values('%s',%s,'%s',%s,%s,'%s','%s')" % (
            self.target, result[0], result[1], result[2], internal_length, child_number, "-".join(f_sequence),
            "-".join(source_sequence))
    ...
    def link_units(self):
        ...
        results = self.cursor.fetchall()
        for result in results:
            sql2 = "select l2_parent from %s where data_unit_id='%s' and ktid=%s and source_ip='%s' and l2_parent is not null" % (
            self.source, result[1], result[2], result[3])
            self.cursor.execute(sql2)
            results2 = self.cursor.fetchall()
            if len(results2) != 0:
                ...
                if len(results3) != 1:
                    ...
                    sql4 = "select id from %s where data_unit_id='%s' and ktid=%s and source_ip='%s'" % (
                    self.target, results3[0][0], results3[0][1], results3[0][2])
                    self.cursor.execute(sql4)
