...
class Aggregator(object):
    ...
    def startTrace(self):
        ...
        for result in results:
            sql2 = "insert into all_trace_events(source_type,event_id,ftype,data_id,data_unit_id,pid,tid,ktid,ltime,result,source_ip) values(%s,%s,%s,'%s','%s',%s,%s,%s,%s,%s,'%s')" % (
            1, result[0], result[1], result[2], result[3], result[4], result[5], result[6], result[7], result[8],
            result[9])
    ...
    def startThreads(self):
        ...
        for result in results:
            ...
            if result[2] == result[5]:
                ftype = 23
            sql2 = "insert into all_trace_events(source_type,event_id,ftype,data_id,data_unit_id,pid,tid,ktid,ltime,result,source_ip) values(%s,%s,%s,'%s','%s',%s,%s,%s,%s,%s,'%s')" % (
            2, result[0], ftype, data_id, result[1], result[2], result[3], result[4], result[6], 1, result[7])
            self.cursor.execute(sql2)
        self.db.commit()
    ...
    def startOther(self):
        ...
        for result in results:
            data_id = None
            sql2 = "insert into all_trace_events(source_type,event_id,ftype,data_id,data_unit_id,pid,tid,ktid,ltime,result,source_ip) values(%s,%s,%s,'%s','%s',%s,%s,%s,%s,%s,'%s')" % (
            3, result[0], result[1], data_id, result[2], result[3], result[4], result[5], result[6], result[7],
            result[8])
            self.cursor.execute(sql2)
        self.db.commit()
    ...
    def reOrder(self):
        ...
        results = self.cursor.fetchall()
        for result in results:
            sql2 = "insert into combination1(source_type,event_id,ftype,data_id,data_unit_id,pid,tid,ktid,ltime,result,source_ip) values(%s,%s,%s,'%s','%s',%s,%s,%s,%s,%s,'%s')" % (
