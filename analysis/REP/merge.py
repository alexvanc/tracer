...
class Merge():
    ...
    def check_internal(self):
        ...
        for result in results:
            sql2 = "select id, ltime from %s where data_unit_id='%s' and ktid=%s and source_ip='%s' order by ltime asc" % (
                self.target, result[0], result[1], result[2])
            ...
            for result2 in results2:
                ...
                self.db.commit()
                couter = couter + 1
            sql3 = "update %s set l2_length=%s where data_unit_id='%s' and ktid=%s and source_ip='%s' and l2_index=0" % (
                self.target, couter, result[0], result[1], result[2])
            self.cursor.execute(sql3)
            self.db.commit()
        print("Done internal: %s " % couter)
    ...
    def check_after_write(self):
        ...
        results = self.cursor.fetchall()
        for result in results:
            sql2 = "select id from %s where ktid=%s and data_unit_id<>'%s' and data_id='%s' and source_ip='%s'" % (
                self.target, result[2], result[1], result[1], result[3])
            self.cursor.execute(sql2)
            results2 = self.cursor.fetchall()
            number = len(results2)
    ...
    def check_thread(self):
        ...
        results = self.cursor.fetchall()
        for result in results:
            sql2 = "select id from threads where k_thread_id=%s and data_unit_id='%s' and source_ip='%s'" % (
    ...
    def check_network(self):
        ...
        for result in results:
            if result[1] % 2 != 0:
                sql2 = "update %s set l2_parent=%s where data_id='%s' and id<>%s" % (
                    self.target, result[0], result[2], result[0])
                self.cursor.execute(sql2)
                self.db.commit()
        print("Done network")
    ...
    def check_timeOrder(self):
        ...
        results = self.cursor.fetchall()
        for result in results:
            sql2 = "select id from %s where data_unit_id='%s' and ktid=%s and source_ip='%s' order by ltime asc" % (
                self.target, result[0], result[1], result[2])
            self.cursor.execute(sql2)
            results2 = self.cursor.fetchall()
            time_parent_id = 0
    ...
    def check_trace(self):
        ...
        counter = 0
        for result in results:
            sql2 = "select id,l2_index,l2_index_alt,l2_parent from %s where data_unit_id='%s' and ktid=%s and source_ip='%s'" % (
                self.target, result[0], result[1], result[2])
            self.cursor.execute(sql2)
            results2 = self.cursor.fetchall()
            for result2 in results2:
    ...
    def check_trace2(self):
        ...
        counter = 0
        for result in results:
            sql2 = "select id,l2_index,l2_index_alt,l2_parent from %s where data_unit_id='%s' and ktid=%s and source_ip='%s' and l2_parent is not null" % (
                self.target, result[0], result[1], result[2])
            self.cursor.execute(sql2)
            results2 = self.cursor.fetchall()
            if len(results2) == 0:
                print(result)
                counter = counter + 1
        print("the units without parents: %s" % counter)
    ...
    def check_unit_parent(self):
        ...
        counter = 0
        for result in results:
            sql2 = "select id,l2_parent from %s where data_unit_id='%s' and ktid=%s and source_ip='%s' and l2_parent is not null" % (
                self.target, result[0], result[1], result[2])
            self.cursor.execute(sql2)
            results2 = self.cursor.fetchall()
            if len(results2) > 1:
                ...
                for result2 in results2:
                    if result2[1] != parent:
                        counter = counter + 1
                        print(result)
        print("units have multiple parents: %s" % counter)
